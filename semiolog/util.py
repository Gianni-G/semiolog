...
def bin_seg_f_sum(l_units, r_units, voc):
    l_unit = l_units[-1]
    r_unit = r_units[-1]
    return (voc[l_unit] + voc[r_unit])
def bin_seg_f_logLen(l_units, r_units, voc):
    l_unit = l_units[-1]
    r_unit = r_units[-1]
...
def binary_segment(
    segmented_chain, voc, interval=None, bin_seg_f=bin_seg_f_sum
):
    ...
    for pair in seg_sent_pairs:
        cut_scores.append([bin_seg_f(pair[0], pair[1], voc), pair])
...
#     start = time.perf_counter()
#     encoded_forest = util_g.multithreading(tree_encoder, trees)
#     finish = time.perf_counter()
...
#     util_g.list2csv(encoded_forest, filename, paths.segmentations)
...
#                 segs = util_g.multiprocessing(functools.partial(chain2tree, voc=voc, weight_f=weight_f, v_rank = voc_rank, zipf_factor = zipf_factor), chain_list)
#             else:
...
#     start = time.perf_counter()
#     decoded_trees = util_g.multiprocessing(tree_decoder, encoded_trees)
#     finish = time.perf_counter()
...
#     loaded_trees = util_g.csv2list(filename, paths.segmentations, n_start, n_end)
...
#         bigrams += util_g.subsequences(chain.split(), 2)
...
#     siblings = util_g.multithreading(extract_siblings,forest)
...
#     terms_dict = util_g.marginalize(orthogonals,"left")
#     contexts_dict = util_g.marginalize(orthogonals,"right")
...
#     orthogonals,
#     measure = mm_no_modif):
#     results = util_g.multithreading(functools.partial(measure, contexts, orthogonals), terms)
#     return results
...
#     start = time.perf_counter()
#     if normalizeQ:
#         orthogonals = util_g.normalize_dict(orthogonals)
...
#     start = time.perf_counter()
#     pmi_matrix = util_g.pmi(term_context_matrix,alpha=alpha,type_pmi=type)
#     finish = time.perf_counter()
#     if normalizeQ:
...
#         disc_rows = util_g.multithreading(
...
#             csr_indices = util_g.multithreading(row_sets_to_matrix,enumerate(list(set_of_sets)))
#         else:
#             csr_indices = [row_sets_to_matrix(row) for row in enumerate(list(set_of_sets))]
#         csr_indices = np.array(util_g.flatten(csr_indices))
...
#         if parallel:
#             n_subset_M = util_g.multithreading(functools.partial(indiv_intersection, n_subset_M, reduce_thres), matrix_as_sets)
#         else:
...
#         start_st = time.perf_counter()
#         # n_subset_M = util_g.sp_unique(csr_matrix(vstack(n_subset_M_n)))
...
#     matrix_list_ps = util_g.parallel_processes(
#         [
...
#         biorthos = util_g.multithreading(
...
#     bo_M = util_g.parallel_processes(
#         [
#             [biorthogonal,o,oo]
#             for o,oo in
#             list_pairs_matrices
...
#     """
#     bo_oR_ = util_g.parallel_processes(
#         [
...
#                 for lT,rT in util_g.take(n_end, csv_reader)
#             ]
...
#         oT_indiv = util_g.delete_duplicates(types_list_filter_dict[boT])
