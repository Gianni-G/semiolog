...
class PMI_SLG:
    ...
    def __init__(self, semiotic) -> None:
        ...
        self.ng_prob = {ng_n:{k:v*self.norm_factor for k,v in semiotic.vocab.__getattribute__(att).freq.items()} for ng_n,att in enumerate([attribute for attribute in dir(semiotic.vocab) if attribute.startswith("ng")])}
        self.thres = semiotic.syntagmatic.thres
    ...
    def chain2seq(
        ...
        if lSt > len(ng_prob):
            ...
            normalizer = ng_prob[lSt-1].get(string,0)

        for edge in seg_graph_full.edges:
            label = seg_graph_full.edges[edge]["label"]
            score = ng_prob[len(label)-1].get(label,0)-normalizer
